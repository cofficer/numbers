...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
  ...
  % save('2018-03-11_cluster_mask_pla-lora_powspctrm.mat','idx_mask')
  ...
  for ipart = 1:38
    ...
    dat.nocebo_rest(ist:ien) = nocebo_rest(ipart,1:len_freq);
    dat.placebo_rest(ist:ien) = placebo_rest(ipart,1:len_freq);
    dat.lora_rest(ist:ien) = lora_rest(ipart,1:len_freq);
  ...
  ien = len_freq
  for ipart = 1:36
    dat.nocebo_trl(ist:ien) = nocebo_trl(ipart,1:len_freq);
    dat.placebo_trl(ist:ien) = placebo_trl(ipart,1:len_freq);
    dat.lora_trl(ist:ien) = lora_trl(ipart,1:len_freq);
  ...
  dat.parts_rest       = [1:38]'*repmat(1,1,len_freq);
  dat.parts_rest       = dat.parts_rest';
  dat.parts_rest       = dat.parts_rest(:);

  dat.parts_trl       = [1:36]'*repmat(1,1,len_freq);
  dat.parts_trl       = dat.parts_trl';
  dat.parts_trl       = dat.parts_trl(:);

  dat.xfreq_rest       = repmat(freq.freq(1:len_freq),1,38)';
  dat.xfreq_rest       = dat.xfreq_rest';
  dat.xfreq_rest       = dat.xfreq_rest(:);

  dat.xfreq_trl       = repmat(freq.freq(1:len_freq),1,36)';
  ...
  %Cluster-based permutation test on timecourses from Matching project.
    ...
    lorazepam.freq = freq.freq(:,1:len_freq);
    placebo.freq = freq.freq(:,1:len_freq);

    lorazepam.powspctrm = zeros(36,1,len_freq);
    placebo.powspctrm = zeros(36,1,len_freq);

    lorazepam.powspctrm(:,1,:) = lora_trl(:,1:len_freq);
    placebo.powspctrm(:,1,:) = placebo_trl(:,1:len_freq);
  ...
  cfg.tail             = 0;
  cfg.clustertail      = 0;
  cfg.alpha            = 0.05; %for twosided test
